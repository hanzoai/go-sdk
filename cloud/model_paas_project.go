/*
Hanzo Cloud — Unified API (V8 · Open Edition)

The single unified OpenAPI surface for ALL Hanzo services, aggregated from the per-service specs. Every route is https://api.hanzo.ai/v1/<service>/_*. Grouped by the canonical domains in capabilities.yaml (the ONE registry). Regenerated by merge.py — edit the per-service <svc>/openapi.yaml + capabilities.yaml, not this file.

API version: 8.0.0
Contact: support@hanzo.ai
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package cloud

import (
	"encoding/json"
	"time"
)

// checks if the PaasProject type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PaasProject{}

// PaasProject struct for PaasProject
type PaasProject struct {
	Id *string `json:"_id,omitempty"`
	Iid *string `json:"iid,omitempty"`
	OrgId *string `json:"orgId,omitempty"`
	Name *string `json:"name,omitempty"`
	Color *string `json:"color,omitempty"`
	Team []PaasProjectTeamInner `json:"team,omitempty"`
	Environments []PaasEnvironment `json:"environments,omitempty"`
	CreatedAt *time.Time `json:"createdAt,omitempty"`
}

// NewPaasProject instantiates a new PaasProject object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPaasProject() *PaasProject {
	this := PaasProject{}
	return &this
}

// NewPaasProjectWithDefaults instantiates a new PaasProject object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPaasProjectWithDefaults() *PaasProject {
	this := PaasProject{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *PaasProject) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *PaasProject) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *PaasProject) SetId(v string) {
	o.Id = &v
}

// GetIid returns the Iid field value if set, zero value otherwise.
func (o *PaasProject) GetIid() string {
	if o == nil || IsNil(o.Iid) {
		var ret string
		return ret
	}
	return *o.Iid
}

// GetIidOk returns a tuple with the Iid field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetIidOk() (*string, bool) {
	if o == nil || IsNil(o.Iid) {
		return nil, false
	}
	return o.Iid, true
}

// HasIid returns a boolean if a field has been set.
func (o *PaasProject) HasIid() bool {
	if o != nil && !IsNil(o.Iid) {
		return true
	}

	return false
}

// SetIid gets a reference to the given string and assigns it to the Iid field.
func (o *PaasProject) SetIid(v string) {
	o.Iid = &v
}

// GetOrgId returns the OrgId field value if set, zero value otherwise.
func (o *PaasProject) GetOrgId() string {
	if o == nil || IsNil(o.OrgId) {
		var ret string
		return ret
	}
	return *o.OrgId
}

// GetOrgIdOk returns a tuple with the OrgId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetOrgIdOk() (*string, bool) {
	if o == nil || IsNil(o.OrgId) {
		return nil, false
	}
	return o.OrgId, true
}

// HasOrgId returns a boolean if a field has been set.
func (o *PaasProject) HasOrgId() bool {
	if o != nil && !IsNil(o.OrgId) {
		return true
	}

	return false
}

// SetOrgId gets a reference to the given string and assigns it to the OrgId field.
func (o *PaasProject) SetOrgId(v string) {
	o.OrgId = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *PaasProject) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *PaasProject) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *PaasProject) SetName(v string) {
	o.Name = &v
}

// GetColor returns the Color field value if set, zero value otherwise.
func (o *PaasProject) GetColor() string {
	if o == nil || IsNil(o.Color) {
		var ret string
		return ret
	}
	return *o.Color
}

// GetColorOk returns a tuple with the Color field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetColorOk() (*string, bool) {
	if o == nil || IsNil(o.Color) {
		return nil, false
	}
	return o.Color, true
}

// HasColor returns a boolean if a field has been set.
func (o *PaasProject) HasColor() bool {
	if o != nil && !IsNil(o.Color) {
		return true
	}

	return false
}

// SetColor gets a reference to the given string and assigns it to the Color field.
func (o *PaasProject) SetColor(v string) {
	o.Color = &v
}

// GetTeam returns the Team field value if set, zero value otherwise.
func (o *PaasProject) GetTeam() []PaasProjectTeamInner {
	if o == nil || IsNil(o.Team) {
		var ret []PaasProjectTeamInner
		return ret
	}
	return o.Team
}

// GetTeamOk returns a tuple with the Team field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetTeamOk() ([]PaasProjectTeamInner, bool) {
	if o == nil || IsNil(o.Team) {
		return nil, false
	}
	return o.Team, true
}

// HasTeam returns a boolean if a field has been set.
func (o *PaasProject) HasTeam() bool {
	if o != nil && !IsNil(o.Team) {
		return true
	}

	return false
}

// SetTeam gets a reference to the given []PaasProjectTeamInner and assigns it to the Team field.
func (o *PaasProject) SetTeam(v []PaasProjectTeamInner) {
	o.Team = v
}

// GetEnvironments returns the Environments field value if set, zero value otherwise.
func (o *PaasProject) GetEnvironments() []PaasEnvironment {
	if o == nil || IsNil(o.Environments) {
		var ret []PaasEnvironment
		return ret
	}
	return o.Environments
}

// GetEnvironmentsOk returns a tuple with the Environments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetEnvironmentsOk() ([]PaasEnvironment, bool) {
	if o == nil || IsNil(o.Environments) {
		return nil, false
	}
	return o.Environments, true
}

// HasEnvironments returns a boolean if a field has been set.
func (o *PaasProject) HasEnvironments() bool {
	if o != nil && !IsNil(o.Environments) {
		return true
	}

	return false
}

// SetEnvironments gets a reference to the given []PaasEnvironment and assigns it to the Environments field.
func (o *PaasProject) SetEnvironments(v []PaasEnvironment) {
	o.Environments = v
}

// GetCreatedAt returns the CreatedAt field value if set, zero value otherwise.
func (o *PaasProject) GetCreatedAt() time.Time {
	if o == nil || IsNil(o.CreatedAt) {
		var ret time.Time
		return ret
	}
	return *o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PaasProject) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.CreatedAt) {
		return nil, false
	}
	return o.CreatedAt, true
}

// HasCreatedAt returns a boolean if a field has been set.
func (o *PaasProject) HasCreatedAt() bool {
	if o != nil && !IsNil(o.CreatedAt) {
		return true
	}

	return false
}

// SetCreatedAt gets a reference to the given time.Time and assigns it to the CreatedAt field.
func (o *PaasProject) SetCreatedAt(v time.Time) {
	o.CreatedAt = &v
}

func (o PaasProject) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PaasProject) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["_id"] = o.Id
	}
	if !IsNil(o.Iid) {
		toSerialize["iid"] = o.Iid
	}
	if !IsNil(o.OrgId) {
		toSerialize["orgId"] = o.OrgId
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Color) {
		toSerialize["color"] = o.Color
	}
	if !IsNil(o.Team) {
		toSerialize["team"] = o.Team
	}
	if !IsNil(o.Environments) {
		toSerialize["environments"] = o.Environments
	}
	if !IsNil(o.CreatedAt) {
		toSerialize["createdAt"] = o.CreatedAt
	}
	return toSerialize, nil
}

type NullablePaasProject struct {
	value *PaasProject
	isSet bool
}

func (v NullablePaasProject) Get() *PaasProject {
	return v.value
}

func (v *NullablePaasProject) Set(val *PaasProject) {
	v.value = val
	v.isSet = true
}

func (v NullablePaasProject) IsSet() bool {
	return v.isSet
}

func (v *NullablePaasProject) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePaasProject(val *PaasProject) *NullablePaasProject {
	return &NullablePaasProject{value: val, isSet: true}
}

func (v NullablePaasProject) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePaasProject) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


