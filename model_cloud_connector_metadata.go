/*
Hanzo Cloud — Unified API (V8 · Open Edition)

The single unified OpenAPI surface for ALL Hanzo services, aggregated from the per-service specs. Every route is https://api.hanzo.ai/v1/<service>/_*. Grouped by the canonical domains in capabilities.yaml (the ONE registry). cloud/openapi.yaml is hanzoai/cloud's own woven document, copied verbatim by sync.py, and it wins wherever it and a hand-written spec describe the same route. Regenerated by merge.py — edit the per-service <svc>/openapi.yaml + capabilities.yaml, not this file.

API version: 8.0.0
Contact: support@hanzo.ai
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the CloudConnectorMetadata type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CloudConnectorMetadata{}

// CloudConnectorMetadata struct for CloudConnectorMetadata
type CloudConnectorMetadata struct {
	Actions     []CloudConnectorAction  `json:"actions,omitempty"`
	Auth        *CloudConnectorAuth     `json:"auth,omitempty"`
	Categories  []string                `json:"categories,omitempty"`
	Description *string                 `json:"description,omitempty"`
	DisplayName *string                 `json:"displayName,omitempty"`
	LogoUrl     *string                 `json:"logoUrl,omitempty"`
	Name        *string                 `json:"name,omitempty"`
	Triggers    []CloudConnectorTrigger `json:"triggers,omitempty"`
	Version     *string                 `json:"version,omitempty"`
}

// NewCloudConnectorMetadata instantiates a new CloudConnectorMetadata object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCloudConnectorMetadata() *CloudConnectorMetadata {
	this := CloudConnectorMetadata{}
	return &this
}

// NewCloudConnectorMetadataWithDefaults instantiates a new CloudConnectorMetadata object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCloudConnectorMetadataWithDefaults() *CloudConnectorMetadata {
	this := CloudConnectorMetadata{}
	return &this
}

// GetActions returns the Actions field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetActions() []CloudConnectorAction {
	if o == nil || IsNil(o.Actions) {
		var ret []CloudConnectorAction
		return ret
	}
	return o.Actions
}

// GetActionsOk returns a tuple with the Actions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetActionsOk() ([]CloudConnectorAction, bool) {
	if o == nil || IsNil(o.Actions) {
		return nil, false
	}
	return o.Actions, true
}

// HasActions returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasActions() bool {
	if o != nil && !IsNil(o.Actions) {
		return true
	}

	return false
}

// SetActions gets a reference to the given []CloudConnectorAction and assigns it to the Actions field.
func (o *CloudConnectorMetadata) SetActions(v []CloudConnectorAction) {
	o.Actions = v
}

// GetAuth returns the Auth field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetAuth() CloudConnectorAuth {
	if o == nil || IsNil(o.Auth) {
		var ret CloudConnectorAuth
		return ret
	}
	return *o.Auth
}

// GetAuthOk returns a tuple with the Auth field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetAuthOk() (*CloudConnectorAuth, bool) {
	if o == nil || IsNil(o.Auth) {
		return nil, false
	}
	return o.Auth, true
}

// HasAuth returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasAuth() bool {
	if o != nil && !IsNil(o.Auth) {
		return true
	}

	return false
}

// SetAuth gets a reference to the given CloudConnectorAuth and assigns it to the Auth field.
func (o *CloudConnectorMetadata) SetAuth(v CloudConnectorAuth) {
	o.Auth = &v
}

// GetCategories returns the Categories field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetCategories() []string {
	if o == nil || IsNil(o.Categories) {
		var ret []string
		return ret
	}
	return o.Categories
}

// GetCategoriesOk returns a tuple with the Categories field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetCategoriesOk() ([]string, bool) {
	if o == nil || IsNil(o.Categories) {
		return nil, false
	}
	return o.Categories, true
}

// HasCategories returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasCategories() bool {
	if o != nil && !IsNil(o.Categories) {
		return true
	}

	return false
}

// SetCategories gets a reference to the given []string and assigns it to the Categories field.
func (o *CloudConnectorMetadata) SetCategories(v []string) {
	o.Categories = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CloudConnectorMetadata) SetDescription(v string) {
	o.Description = &v
}

// GetDisplayName returns the DisplayName field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetDisplayName() string {
	if o == nil || IsNil(o.DisplayName) {
		var ret string
		return ret
	}
	return *o.DisplayName
}

// GetDisplayNameOk returns a tuple with the DisplayName field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetDisplayNameOk() (*string, bool) {
	if o == nil || IsNil(o.DisplayName) {
		return nil, false
	}
	return o.DisplayName, true
}

// HasDisplayName returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasDisplayName() bool {
	if o != nil && !IsNil(o.DisplayName) {
		return true
	}

	return false
}

// SetDisplayName gets a reference to the given string and assigns it to the DisplayName field.
func (o *CloudConnectorMetadata) SetDisplayName(v string) {
	o.DisplayName = &v
}

// GetLogoUrl returns the LogoUrl field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetLogoUrl() string {
	if o == nil || IsNil(o.LogoUrl) {
		var ret string
		return ret
	}
	return *o.LogoUrl
}

// GetLogoUrlOk returns a tuple with the LogoUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetLogoUrlOk() (*string, bool) {
	if o == nil || IsNil(o.LogoUrl) {
		return nil, false
	}
	return o.LogoUrl, true
}

// HasLogoUrl returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasLogoUrl() bool {
	if o != nil && !IsNil(o.LogoUrl) {
		return true
	}

	return false
}

// SetLogoUrl gets a reference to the given string and assigns it to the LogoUrl field.
func (o *CloudConnectorMetadata) SetLogoUrl(v string) {
	o.LogoUrl = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *CloudConnectorMetadata) SetName(v string) {
	o.Name = &v
}

// GetTriggers returns the Triggers field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetTriggers() []CloudConnectorTrigger {
	if o == nil || IsNil(o.Triggers) {
		var ret []CloudConnectorTrigger
		return ret
	}
	return o.Triggers
}

// GetTriggersOk returns a tuple with the Triggers field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetTriggersOk() ([]CloudConnectorTrigger, bool) {
	if o == nil || IsNil(o.Triggers) {
		return nil, false
	}
	return o.Triggers, true
}

// HasTriggers returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasTriggers() bool {
	if o != nil && !IsNil(o.Triggers) {
		return true
	}

	return false
}

// SetTriggers gets a reference to the given []CloudConnectorTrigger and assigns it to the Triggers field.
func (o *CloudConnectorMetadata) SetTriggers(v []CloudConnectorTrigger) {
	o.Triggers = v
}

// GetVersion returns the Version field value if set, zero value otherwise.
func (o *CloudConnectorMetadata) GetVersion() string {
	if o == nil || IsNil(o.Version) {
		var ret string
		return ret
	}
	return *o.Version
}

// GetVersionOk returns a tuple with the Version field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudConnectorMetadata) GetVersionOk() (*string, bool) {
	if o == nil || IsNil(o.Version) {
		return nil, false
	}
	return o.Version, true
}

// HasVersion returns a boolean if a field has been set.
func (o *CloudConnectorMetadata) HasVersion() bool {
	if o != nil && !IsNil(o.Version) {
		return true
	}

	return false
}

// SetVersion gets a reference to the given string and assigns it to the Version field.
func (o *CloudConnectorMetadata) SetVersion(v string) {
	o.Version = &v
}

func (o CloudConnectorMetadata) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CloudConnectorMetadata) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Actions) {
		toSerialize["actions"] = o.Actions
	}
	if !IsNil(o.Auth) {
		toSerialize["auth"] = o.Auth
	}
	if !IsNil(o.Categories) {
		toSerialize["categories"] = o.Categories
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.DisplayName) {
		toSerialize["displayName"] = o.DisplayName
	}
	if !IsNil(o.LogoUrl) {
		toSerialize["logoUrl"] = o.LogoUrl
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Triggers) {
		toSerialize["triggers"] = o.Triggers
	}
	if !IsNil(o.Version) {
		toSerialize["version"] = o.Version
	}
	return toSerialize, nil
}

type NullableCloudConnectorMetadata struct {
	value *CloudConnectorMetadata
	isSet bool
}

func (v NullableCloudConnectorMetadata) Get() *CloudConnectorMetadata {
	return v.value
}

func (v *NullableCloudConnectorMetadata) Set(val *CloudConnectorMetadata) {
	v.value = val
	v.isSet = true
}

func (v NullableCloudConnectorMetadata) IsSet() bool {
	return v.isSet
}

func (v *NullableCloudConnectorMetadata) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCloudConnectorMetadata(val *CloudConnectorMetadata) *NullableCloudConnectorMetadata {
	return &NullableCloudConnectorMetadata{value: val, isSet: true}
}

func (v NullableCloudConnectorMetadata) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCloudConnectorMetadata) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
