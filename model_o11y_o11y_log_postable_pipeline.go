/*
Hanzo Cloud API

Composed from each subsystem's own projection of its router, in the fleet's mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the O11yO11yLogPostablePipeline type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &O11yO11yLogPostablePipeline{}

// O11yO11yLogPostablePipeline struct for O11yO11yLogPostablePipeline
type O11yO11yLogPostablePipeline struct {
	// Alias is the pipeline's short name.
	Alias *string `json:"alias,omitempty"`
	// Config is the pipeline's processors, in order.
	Config []O11yO11yLogPipelineOperator `json:"config,omitempty"`
	// Description says what the pipeline is for.
	Description *string `json:"description,omitempty"`
	// Enabled turns the pipeline on.
	Enabled *bool `json:"enabled,omitempty"`
	// Filter selects which records the pipeline processes.
	Filter *O11yO11yLogFilter `json:"filter,omitempty"`
	// ID is the pipeline's id. Empty on a new pipeline; the id it was listed with to keep an existing one.
	Id *string `json:"id,omitempty"`
	// Name is the pipeline's display name.
	Name *string `json:"name,omitempty"`
	// OrderID is the pipeline's 1-based position in the set.
	OrderId *int32 `json:"orderId,omitempty"`
}

// NewO11yO11yLogPostablePipeline instantiates a new O11yO11yLogPostablePipeline object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewO11yO11yLogPostablePipeline() *O11yO11yLogPostablePipeline {
	this := O11yO11yLogPostablePipeline{}
	return &this
}

// NewO11yO11yLogPostablePipelineWithDefaults instantiates a new O11yO11yLogPostablePipeline object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewO11yO11yLogPostablePipelineWithDefaults() *O11yO11yLogPostablePipeline {
	this := O11yO11yLogPostablePipeline{}
	return &this
}

// GetAlias returns the Alias field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetAlias() string {
	if o == nil || IsNil(o.Alias) {
		var ret string
		return ret
	}
	return *o.Alias
}

// GetAliasOk returns a tuple with the Alias field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetAliasOk() (*string, bool) {
	if o == nil || IsNil(o.Alias) {
		return nil, false
	}
	return o.Alias, true
}

// HasAlias returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasAlias() bool {
	if o != nil && !IsNil(o.Alias) {
		return true
	}

	return false
}

// SetAlias gets a reference to the given string and assigns it to the Alias field.
func (o *O11yO11yLogPostablePipeline) SetAlias(v string) {
	o.Alias = &v
}

// GetConfig returns the Config field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetConfig() []O11yO11yLogPipelineOperator {
	if o == nil || IsNil(o.Config) {
		var ret []O11yO11yLogPipelineOperator
		return ret
	}
	return o.Config
}

// GetConfigOk returns a tuple with the Config field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetConfigOk() ([]O11yO11yLogPipelineOperator, bool) {
	if o == nil || IsNil(o.Config) {
		return nil, false
	}
	return o.Config, true
}

// HasConfig returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasConfig() bool {
	if o != nil && !IsNil(o.Config) {
		return true
	}

	return false
}

// SetConfig gets a reference to the given []O11yO11yLogPipelineOperator and assigns it to the Config field.
func (o *O11yO11yLogPostablePipeline) SetConfig(v []O11yO11yLogPipelineOperator) {
	o.Config = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *O11yO11yLogPostablePipeline) SetDescription(v string) {
	o.Description = &v
}

// GetEnabled returns the Enabled field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetEnabled() bool {
	if o == nil || IsNil(o.Enabled) {
		var ret bool
		return ret
	}
	return *o.Enabled
}

// GetEnabledOk returns a tuple with the Enabled field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetEnabledOk() (*bool, bool) {
	if o == nil || IsNil(o.Enabled) {
		return nil, false
	}
	return o.Enabled, true
}

// HasEnabled returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasEnabled() bool {
	if o != nil && !IsNil(o.Enabled) {
		return true
	}

	return false
}

// SetEnabled gets a reference to the given bool and assigns it to the Enabled field.
func (o *O11yO11yLogPostablePipeline) SetEnabled(v bool) {
	o.Enabled = &v
}

// GetFilter returns the Filter field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetFilter() O11yO11yLogFilter {
	if o == nil || IsNil(o.Filter) {
		var ret O11yO11yLogFilter
		return ret
	}
	return *o.Filter
}

// GetFilterOk returns a tuple with the Filter field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetFilterOk() (*O11yO11yLogFilter, bool) {
	if o == nil || IsNil(o.Filter) {
		return nil, false
	}
	return o.Filter, true
}

// HasFilter returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasFilter() bool {
	if o != nil && !IsNil(o.Filter) {
		return true
	}

	return false
}

// SetFilter gets a reference to the given O11yO11yLogFilter and assigns it to the Filter field.
func (o *O11yO11yLogPostablePipeline) SetFilter(v O11yO11yLogFilter) {
	o.Filter = &v
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *O11yO11yLogPostablePipeline) SetId(v string) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *O11yO11yLogPostablePipeline) SetName(v string) {
	o.Name = &v
}

// GetOrderId returns the OrderId field value if set, zero value otherwise.
func (o *O11yO11yLogPostablePipeline) GetOrderId() int32 {
	if o == nil || IsNil(o.OrderId) {
		var ret int32
		return ret
	}
	return *o.OrderId
}

// GetOrderIdOk returns a tuple with the OrderId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yLogPostablePipeline) GetOrderIdOk() (*int32, bool) {
	if o == nil || IsNil(o.OrderId) {
		return nil, false
	}
	return o.OrderId, true
}

// HasOrderId returns a boolean if a field has been set.
func (o *O11yO11yLogPostablePipeline) HasOrderId() bool {
	if o != nil && !IsNil(o.OrderId) {
		return true
	}

	return false
}

// SetOrderId gets a reference to the given int32 and assigns it to the OrderId field.
func (o *O11yO11yLogPostablePipeline) SetOrderId(v int32) {
	o.OrderId = &v
}

func (o O11yO11yLogPostablePipeline) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o O11yO11yLogPostablePipeline) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Alias) {
		toSerialize["alias"] = o.Alias
	}
	if !IsNil(o.Config) {
		toSerialize["config"] = o.Config
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Enabled) {
		toSerialize["enabled"] = o.Enabled
	}
	if !IsNil(o.Filter) {
		toSerialize["filter"] = o.Filter
	}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.OrderId) {
		toSerialize["orderId"] = o.OrderId
	}
	return toSerialize, nil
}

type NullableO11yO11yLogPostablePipeline struct {
	value *O11yO11yLogPostablePipeline
	isSet bool
}

func (v NullableO11yO11yLogPostablePipeline) Get() *O11yO11yLogPostablePipeline {
	return v.value
}

func (v *NullableO11yO11yLogPostablePipeline) Set(val *O11yO11yLogPostablePipeline) {
	v.value = val
	v.isSet = true
}

func (v NullableO11yO11yLogPostablePipeline) IsSet() bool {
	return v.isSet
}

func (v *NullableO11yO11yLogPostablePipeline) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableO11yO11yLogPostablePipeline(val *O11yO11yLogPostablePipeline) *NullableO11yO11yLogPostablePipeline {
	return &NullableO11yO11yLogPostablePipeline{value: val, isSet: true}
}

func (v NullableO11yO11yLogPostablePipeline) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableO11yO11yLogPostablePipeline) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
