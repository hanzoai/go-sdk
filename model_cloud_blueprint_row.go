/*
Hanzo Cloud — Unified API (V8 · Open Edition)

The single unified OpenAPI surface for ALL Hanzo services, aggregated from the per-service specs. Every route is https://api.hanzo.ai/v1/<service>/_*. Grouped by the canonical domains in capabilities.yaml (the ONE registry). cloud/openapi.yaml is hanzoai/cloud's own woven document, copied verbatim by sync.py, and it wins wherever it and a hand-written spec describe the same route. Regenerated by merge.py — edit the per-service <svc>/openapi.yaml + capabilities.yaml, not this file.

API version: 8.0.0
Contact: support@hanzo.ai
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the CloudBlueprintRow type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CloudBlueprintRow{}

// CloudBlueprintRow struct for CloudBlueprintRow
type CloudBlueprintRow struct {
	// CentsPerMonth is the estimated compute cost of running the whole stack for one month, in USD cents, from the rate card GET /v1/blueprint/health echoes.
	EstCentsPerMonth *int32 `json:"estCentsPerMonth,omitempty"`
	// Services is how many compose services the stack runs.
	Services *int32 `json:"services,omitempty"`
	// TemplateID is the blueprint slug — the id GET /v1/blueprint/sbom takes as ?template= and the path under templates.hanzo.ai/blueprints/<id>/.
	TemplateId *string `json:"templateId,omitempty"`
}

// NewCloudBlueprintRow instantiates a new CloudBlueprintRow object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCloudBlueprintRow() *CloudBlueprintRow {
	this := CloudBlueprintRow{}
	return &this
}

// NewCloudBlueprintRowWithDefaults instantiates a new CloudBlueprintRow object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCloudBlueprintRowWithDefaults() *CloudBlueprintRow {
	this := CloudBlueprintRow{}
	return &this
}

// GetEstCentsPerMonth returns the EstCentsPerMonth field value if set, zero value otherwise.
func (o *CloudBlueprintRow) GetEstCentsPerMonth() int32 {
	if o == nil || IsNil(o.EstCentsPerMonth) {
		var ret int32
		return ret
	}
	return *o.EstCentsPerMonth
}

// GetEstCentsPerMonthOk returns a tuple with the EstCentsPerMonth field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudBlueprintRow) GetEstCentsPerMonthOk() (*int32, bool) {
	if o == nil || IsNil(o.EstCentsPerMonth) {
		return nil, false
	}
	return o.EstCentsPerMonth, true
}

// HasEstCentsPerMonth returns a boolean if a field has been set.
func (o *CloudBlueprintRow) HasEstCentsPerMonth() bool {
	if o != nil && !IsNil(o.EstCentsPerMonth) {
		return true
	}

	return false
}

// SetEstCentsPerMonth gets a reference to the given int32 and assigns it to the EstCentsPerMonth field.
func (o *CloudBlueprintRow) SetEstCentsPerMonth(v int32) {
	o.EstCentsPerMonth = &v
}

// GetServices returns the Services field value if set, zero value otherwise.
func (o *CloudBlueprintRow) GetServices() int32 {
	if o == nil || IsNil(o.Services) {
		var ret int32
		return ret
	}
	return *o.Services
}

// GetServicesOk returns a tuple with the Services field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudBlueprintRow) GetServicesOk() (*int32, bool) {
	if o == nil || IsNil(o.Services) {
		return nil, false
	}
	return o.Services, true
}

// HasServices returns a boolean if a field has been set.
func (o *CloudBlueprintRow) HasServices() bool {
	if o != nil && !IsNil(o.Services) {
		return true
	}

	return false
}

// SetServices gets a reference to the given int32 and assigns it to the Services field.
func (o *CloudBlueprintRow) SetServices(v int32) {
	o.Services = &v
}

// GetTemplateId returns the TemplateId field value if set, zero value otherwise.
func (o *CloudBlueprintRow) GetTemplateId() string {
	if o == nil || IsNil(o.TemplateId) {
		var ret string
		return ret
	}
	return *o.TemplateId
}

// GetTemplateIdOk returns a tuple with the TemplateId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CloudBlueprintRow) GetTemplateIdOk() (*string, bool) {
	if o == nil || IsNil(o.TemplateId) {
		return nil, false
	}
	return o.TemplateId, true
}

// HasTemplateId returns a boolean if a field has been set.
func (o *CloudBlueprintRow) HasTemplateId() bool {
	if o != nil && !IsNil(o.TemplateId) {
		return true
	}

	return false
}

// SetTemplateId gets a reference to the given string and assigns it to the TemplateId field.
func (o *CloudBlueprintRow) SetTemplateId(v string) {
	o.TemplateId = &v
}

func (o CloudBlueprintRow) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CloudBlueprintRow) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.EstCentsPerMonth) {
		toSerialize["estCentsPerMonth"] = o.EstCentsPerMonth
	}
	if !IsNil(o.Services) {
		toSerialize["services"] = o.Services
	}
	if !IsNil(o.TemplateId) {
		toSerialize["templateId"] = o.TemplateId
	}
	return toSerialize, nil
}

type NullableCloudBlueprintRow struct {
	value *CloudBlueprintRow
	isSet bool
}

func (v NullableCloudBlueprintRow) Get() *CloudBlueprintRow {
	return v.value
}

func (v *NullableCloudBlueprintRow) Set(val *CloudBlueprintRow) {
	v.value = val
	v.isSet = true
}

func (v NullableCloudBlueprintRow) IsSet() bool {
	return v.isSet
}

func (v *NullableCloudBlueprintRow) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCloudBlueprintRow(val *CloudBlueprintRow) *NullableCloudBlueprintRow {
	return &NullableCloudBlueprintRow{value: val, isSet: true}
}

func (v NullableCloudBlueprintRow) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCloudBlueprintRow) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
