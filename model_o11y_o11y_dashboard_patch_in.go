/*
Hanzo Cloud API

Composed from each subsystem's own projection of its router, in the fleet's mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the O11yO11yDashboardPatchIn type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &O11yO11yDashboardPatchIn{}

// O11yO11yDashboardPatchIn struct for O11yO11yDashboardPatchIn
type O11yO11yDashboardPatchIn struct {
	// ID is the dashboard id from the path.
	Id *string `json:"id,omitempty"`
	// Ops are the JSON Patch operations, applied in order. On the wire this IS the request body — a bare array, not an object.
	Ops []O11yO11yDashboardPatchOp `json:"ops,omitempty"`
}

// NewO11yO11yDashboardPatchIn instantiates a new O11yO11yDashboardPatchIn object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewO11yO11yDashboardPatchIn() *O11yO11yDashboardPatchIn {
	this := O11yO11yDashboardPatchIn{}
	return &this
}

// NewO11yO11yDashboardPatchInWithDefaults instantiates a new O11yO11yDashboardPatchIn object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewO11yO11yDashboardPatchInWithDefaults() *O11yO11yDashboardPatchIn {
	this := O11yO11yDashboardPatchIn{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *O11yO11yDashboardPatchIn) GetId() string {
	if o == nil || IsNil(o.Id) {
		var ret string
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yDashboardPatchIn) GetIdOk() (*string, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *O11yO11yDashboardPatchIn) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given string and assigns it to the Id field.
func (o *O11yO11yDashboardPatchIn) SetId(v string) {
	o.Id = &v
}

// GetOps returns the Ops field value if set, zero value otherwise.
func (o *O11yO11yDashboardPatchIn) GetOps() []O11yO11yDashboardPatchOp {
	if o == nil || IsNil(o.Ops) {
		var ret []O11yO11yDashboardPatchOp
		return ret
	}
	return o.Ops
}

// GetOpsOk returns a tuple with the Ops field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *O11yO11yDashboardPatchIn) GetOpsOk() ([]O11yO11yDashboardPatchOp, bool) {
	if o == nil || IsNil(o.Ops) {
		return nil, false
	}
	return o.Ops, true
}

// HasOps returns a boolean if a field has been set.
func (o *O11yO11yDashboardPatchIn) HasOps() bool {
	if o != nil && !IsNil(o.Ops) {
		return true
	}

	return false
}

// SetOps gets a reference to the given []O11yO11yDashboardPatchOp and assigns it to the Ops field.
func (o *O11yO11yDashboardPatchIn) SetOps(v []O11yO11yDashboardPatchOp) {
	o.Ops = v
}

func (o O11yO11yDashboardPatchIn) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o O11yO11yDashboardPatchIn) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Ops) {
		toSerialize["ops"] = o.Ops
	}
	return toSerialize, nil
}

type NullableO11yO11yDashboardPatchIn struct {
	value *O11yO11yDashboardPatchIn
	isSet bool
}

func (v NullableO11yO11yDashboardPatchIn) Get() *O11yO11yDashboardPatchIn {
	return v.value
}

func (v *NullableO11yO11yDashboardPatchIn) Set(val *O11yO11yDashboardPatchIn) {
	v.value = val
	v.isSet = true
}

func (v NullableO11yO11yDashboardPatchIn) IsSet() bool {
	return v.isSet
}

func (v *NullableO11yO11yDashboardPatchIn) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableO11yO11yDashboardPatchIn(val *O11yO11yDashboardPatchIn) *NullableO11yO11yDashboardPatchIn {
	return &NullableO11yO11yDashboardPatchIn{value: val, isSet: true}
}

func (v NullableO11yO11yDashboardPatchIn) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableO11yO11yDashboardPatchIn) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
