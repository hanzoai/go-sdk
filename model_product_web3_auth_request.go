/*
Hanzo Cloud — Unified API (V8 · Open Edition)

The single unified OpenAPI surface for ALL Hanzo services, aggregated from the per-service specs. Every route is https://api.hanzo.ai/v1/<service>/_*. Grouped by the canonical domains in capabilities.yaml (the ONE registry). Regenerated by merge.py — edit the per-service <svc>/openapi.yaml + capabilities.yaml, not this file.

API version: 8.0.0
Contact: support@hanzo.ai
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the ProductWeb3AuthRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ProductWeb3AuthRequest{}

// ProductWeb3AuthRequest Web3 wallet authentication request
type ProductWeb3AuthRequest struct {
	// Ethereum wallet address
	Address string `json:"address" validate:"regexp=^0x[a-fA-F0-9]{40}$"`
	// Nonce obtained from /v1/iam/auth/web3-nonce
	Nonce string `json:"nonce"`
	// Timestamp when signature was created
	CreateAt *int32 `json:"createAt,omitempty"`
	// The message that was signed
	TypedData *string `json:"typedData,omitempty"`
	// Wallet signature
	Signature string `json:"signature"`
	// Type of wallet used
	WalletType *string `json:"walletType,omitempty"`
}

type _ProductWeb3AuthRequest ProductWeb3AuthRequest

// NewProductWeb3AuthRequest instantiates a new ProductWeb3AuthRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewProductWeb3AuthRequest(address string, nonce string, signature string) *ProductWeb3AuthRequest {
	this := ProductWeb3AuthRequest{}
	this.Address = address
	this.Nonce = nonce
	this.Signature = signature
	return &this
}

// NewProductWeb3AuthRequestWithDefaults instantiates a new ProductWeb3AuthRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewProductWeb3AuthRequestWithDefaults() *ProductWeb3AuthRequest {
	this := ProductWeb3AuthRequest{}
	return &this
}

// GetAddress returns the Address field value
func (o *ProductWeb3AuthRequest) GetAddress() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Address
}

// GetAddressOk returns a tuple with the Address field value
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetAddressOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Address, true
}

// SetAddress sets field value
func (o *ProductWeb3AuthRequest) SetAddress(v string) {
	o.Address = v
}

// GetNonce returns the Nonce field value
func (o *ProductWeb3AuthRequest) GetNonce() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Nonce
}

// GetNonceOk returns a tuple with the Nonce field value
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetNonceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Nonce, true
}

// SetNonce sets field value
func (o *ProductWeb3AuthRequest) SetNonce(v string) {
	o.Nonce = v
}

// GetCreateAt returns the CreateAt field value if set, zero value otherwise.
func (o *ProductWeb3AuthRequest) GetCreateAt() int32 {
	if o == nil || IsNil(o.CreateAt) {
		var ret int32
		return ret
	}
	return *o.CreateAt
}

// GetCreateAtOk returns a tuple with the CreateAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetCreateAtOk() (*int32, bool) {
	if o == nil || IsNil(o.CreateAt) {
		return nil, false
	}
	return o.CreateAt, true
}

// HasCreateAt returns a boolean if a field has been set.
func (o *ProductWeb3AuthRequest) HasCreateAt() bool {
	if o != nil && !IsNil(o.CreateAt) {
		return true
	}

	return false
}

// SetCreateAt gets a reference to the given int32 and assigns it to the CreateAt field.
func (o *ProductWeb3AuthRequest) SetCreateAt(v int32) {
	o.CreateAt = &v
}

// GetTypedData returns the TypedData field value if set, zero value otherwise.
func (o *ProductWeb3AuthRequest) GetTypedData() string {
	if o == nil || IsNil(o.TypedData) {
		var ret string
		return ret
	}
	return *o.TypedData
}

// GetTypedDataOk returns a tuple with the TypedData field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetTypedDataOk() (*string, bool) {
	if o == nil || IsNil(o.TypedData) {
		return nil, false
	}
	return o.TypedData, true
}

// HasTypedData returns a boolean if a field has been set.
func (o *ProductWeb3AuthRequest) HasTypedData() bool {
	if o != nil && !IsNil(o.TypedData) {
		return true
	}

	return false
}

// SetTypedData gets a reference to the given string and assigns it to the TypedData field.
func (o *ProductWeb3AuthRequest) SetTypedData(v string) {
	o.TypedData = &v
}

// GetSignature returns the Signature field value
func (o *ProductWeb3AuthRequest) GetSignature() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Signature
}

// GetSignatureOk returns a tuple with the Signature field value
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetSignatureOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Signature, true
}

// SetSignature sets field value
func (o *ProductWeb3AuthRequest) SetSignature(v string) {
	o.Signature = v
}

// GetWalletType returns the WalletType field value if set, zero value otherwise.
func (o *ProductWeb3AuthRequest) GetWalletType() string {
	if o == nil || IsNil(o.WalletType) {
		var ret string
		return ret
	}
	return *o.WalletType
}

// GetWalletTypeOk returns a tuple with the WalletType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ProductWeb3AuthRequest) GetWalletTypeOk() (*string, bool) {
	if o == nil || IsNil(o.WalletType) {
		return nil, false
	}
	return o.WalletType, true
}

// HasWalletType returns a boolean if a field has been set.
func (o *ProductWeb3AuthRequest) HasWalletType() bool {
	if o != nil && !IsNil(o.WalletType) {
		return true
	}

	return false
}

// SetWalletType gets a reference to the given string and assigns it to the WalletType field.
func (o *ProductWeb3AuthRequest) SetWalletType(v string) {
	o.WalletType = &v
}

func (o ProductWeb3AuthRequest) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ProductWeb3AuthRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["address"] = o.Address
	toSerialize["nonce"] = o.Nonce
	if !IsNil(o.CreateAt) {
		toSerialize["createAt"] = o.CreateAt
	}
	if !IsNil(o.TypedData) {
		toSerialize["typedData"] = o.TypedData
	}
	toSerialize["signature"] = o.Signature
	if !IsNil(o.WalletType) {
		toSerialize["walletType"] = o.WalletType
	}
	return toSerialize, nil
}

func (o *ProductWeb3AuthRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"address",
		"nonce",
		"signature",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varProductWeb3AuthRequest := _ProductWeb3AuthRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varProductWeb3AuthRequest)

	if err != nil {
		return err
	}

	*o = ProductWeb3AuthRequest(varProductWeb3AuthRequest)

	return err
}

type NullableProductWeb3AuthRequest struct {
	value *ProductWeb3AuthRequest
	isSet bool
}

func (v NullableProductWeb3AuthRequest) Get() *ProductWeb3AuthRequest {
	return v.value
}

func (v *NullableProductWeb3AuthRequest) Set(val *ProductWeb3AuthRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableProductWeb3AuthRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableProductWeb3AuthRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableProductWeb3AuthRequest(val *ProductWeb3AuthRequest) *NullableProductWeb3AuthRequest {
	return &NullableProductWeb3AuthRequest{value: val, isSet: true}
}

func (v NullableProductWeb3AuthRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableProductWeb3AuthRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
