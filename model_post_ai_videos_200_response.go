/*
Hanzo Cloud API

Composed from each subsystem's own projection of its router, in the fleet's mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the PostAiVideos200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PostAiVideos200Response{}

// PostAiVideos200Response struct for PostAiVideos200Response
type PostAiVideos200Response struct {
	Data  *Video      `json:"data,omitempty"`
	Data2 interface{} `json:"data2,omitempty"`
	// Empty on success, the reason on failure.
	Msg    string `json:"msg"`
	Status string `json:"status"`
}

type _PostAiVideos200Response PostAiVideos200Response

// NewPostAiVideos200Response instantiates a new PostAiVideos200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPostAiVideos200Response(msg string, status string) *PostAiVideos200Response {
	this := PostAiVideos200Response{}
	this.Msg = msg
	this.Status = status
	return &this
}

// NewPostAiVideos200ResponseWithDefaults instantiates a new PostAiVideos200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPostAiVideos200ResponseWithDefaults() *PostAiVideos200Response {
	this := PostAiVideos200Response{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *PostAiVideos200Response) GetData() Video {
	if o == nil || IsNil(o.Data) {
		var ret Video
		return ret
	}
	return *o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *PostAiVideos200Response) GetDataOk() (*Video, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *PostAiVideos200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given Video and assigns it to the Data field.
func (o *PostAiVideos200Response) SetData(v Video) {
	o.Data = &v
}

// GetData2 returns the Data2 field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *PostAiVideos200Response) GetData2() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Data2
}

// GetData2Ok returns a tuple with the Data2 field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *PostAiVideos200Response) GetData2Ok() (*interface{}, bool) {
	if o == nil || IsNil(o.Data2) {
		return nil, false
	}
	return &o.Data2, true
}

// HasData2 returns a boolean if a field has been set.
func (o *PostAiVideos200Response) HasData2() bool {
	if o != nil && !IsNil(o.Data2) {
		return true
	}

	return false
}

// SetData2 gets a reference to the given interface{} and assigns it to the Data2 field.
func (o *PostAiVideos200Response) SetData2(v interface{}) {
	o.Data2 = v
}

// GetMsg returns the Msg field value
func (o *PostAiVideos200Response) GetMsg() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Msg
}

// GetMsgOk returns a tuple with the Msg field value
// and a boolean to check if the value has been set.
func (o *PostAiVideos200Response) GetMsgOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Msg, true
}

// SetMsg sets field value
func (o *PostAiVideos200Response) SetMsg(v string) {
	o.Msg = v
}

// GetStatus returns the Status field value
func (o *PostAiVideos200Response) GetStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *PostAiVideos200Response) GetStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *PostAiVideos200Response) SetStatus(v string) {
	o.Status = v
}

func (o PostAiVideos200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PostAiVideos200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if o.Data2 != nil {
		toSerialize["data2"] = o.Data2
	}
	toSerialize["msg"] = o.Msg
	toSerialize["status"] = o.Status
	return toSerialize, nil
}

func (o *PostAiVideos200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"msg",
		"status",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPostAiVideos200Response := _PostAiVideos200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varPostAiVideos200Response)

	if err != nil {
		return err
	}

	*o = PostAiVideos200Response(varPostAiVideos200Response)

	return err
}

type NullablePostAiVideos200Response struct {
	value *PostAiVideos200Response
	isSet bool
}

func (v NullablePostAiVideos200Response) Get() *PostAiVideos200Response {
	return v.value
}

func (v *NullablePostAiVideos200Response) Set(val *PostAiVideos200Response) {
	v.value = val
	v.isSet = true
}

func (v NullablePostAiVideos200Response) IsSet() bool {
	return v.isSet
}

func (v *NullablePostAiVideos200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePostAiVideos200Response(val *PostAiVideos200Response) *NullablePostAiVideos200Response {
	return &NullablePostAiVideos200Response{value: val, isSet: true}
}

func (v NullablePostAiVideos200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePostAiVideos200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
