/*
Hanzo Cloud API

The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the SpaceList type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SpaceList{}

// SpaceList struct for SpaceList
type SpaceList struct {
	// Spaces are the caller org's spaces, oldest first as the store returns them.
	Spaces []SpaceItem `json:"spaces,omitempty"`
	// Total is how many spaces this org has. It equals len(spaces): the listing is not paged, because one bucket per (org, space) keeps an org's count small by construction, which is the whole reason a drive is a prefix and not a bucket.
	Total *int32 `json:"total,omitempty"`
}

// NewSpaceList instantiates a new SpaceList object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSpaceList() *SpaceList {
	this := SpaceList{}
	return &this
}

// NewSpaceListWithDefaults instantiates a new SpaceList object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSpaceListWithDefaults() *SpaceList {
	this := SpaceList{}
	return &this
}

// GetSpaces returns the Spaces field value if set, zero value otherwise.
func (o *SpaceList) GetSpaces() []SpaceItem {
	if o == nil || IsNil(o.Spaces) {
		var ret []SpaceItem
		return ret
	}
	return o.Spaces
}

// GetSpacesOk returns a tuple with the Spaces field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SpaceList) GetSpacesOk() ([]SpaceItem, bool) {
	if o == nil || IsNil(o.Spaces) {
		return nil, false
	}
	return o.Spaces, true
}

// HasSpaces returns a boolean if a field has been set.
func (o *SpaceList) HasSpaces() bool {
	if o != nil && !IsNil(o.Spaces) {
		return true
	}

	return false
}

// SetSpaces gets a reference to the given []SpaceItem and assigns it to the Spaces field.
func (o *SpaceList) SetSpaces(v []SpaceItem) {
	o.Spaces = v
}

// GetTotal returns the Total field value if set, zero value otherwise.
func (o *SpaceList) GetTotal() int32 {
	if o == nil || IsNil(o.Total) {
		var ret int32
		return ret
	}
	return *o.Total
}

// GetTotalOk returns a tuple with the Total field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SpaceList) GetTotalOk() (*int32, bool) {
	if o == nil || IsNil(o.Total) {
		return nil, false
	}
	return o.Total, true
}

// HasTotal returns a boolean if a field has been set.
func (o *SpaceList) HasTotal() bool {
	if o != nil && !IsNil(o.Total) {
		return true
	}

	return false
}

// SetTotal gets a reference to the given int32 and assigns it to the Total field.
func (o *SpaceList) SetTotal(v int32) {
	o.Total = &v
}

func (o SpaceList) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SpaceList) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Spaces) {
		toSerialize["spaces"] = o.Spaces
	}
	if !IsNil(o.Total) {
		toSerialize["total"] = o.Total
	}
	return toSerialize, nil
}

type NullableSpaceList struct {
	value *SpaceList
	isSet bool
}

func (v NullableSpaceList) Get() *SpaceList {
	return v.value
}

func (v *NullableSpaceList) Set(val *SpaceList) {
	v.value = val
	v.isSet = true
}

func (v NullableSpaceList) IsSet() bool {
	return v.isSet
}

func (v *NullableSpaceList) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSpaceList(val *SpaceList) *NullableSpaceList {
	return &NullableSpaceList{value: val, isSet: true}
}

func (v NullableSpaceList) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSpaceList) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
