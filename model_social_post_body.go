/*
Hanzo Cloud API

The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the SocialPostBody type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SocialPostBody{}

// SocialPostBody struct for SocialPostBody
type SocialPostBody struct {
	// Channel is the network to publish to: x, facebook, instagram, linkedin, tiktok, youtube or threads. Omitted means x.  Example: \"x\"
	Channel *string `json:"channel,omitempty"`
	// Content is the post's text. Required — an empty body is a 400 — and bounded at 8192 characters, comfortably above every network's own limit.  Example: \"Shipping today.\"
	Content *string `json:"content,omitempty"`
	// Media is the post's attached media as URLs, at most 10, each bounded at 1024 characters. Blank entries are dropped. Omitting it CLEARS any stored media.
	Media []string `json:"media,omitempty"`
	// ScheduleAt is when to publish, as a unix timestamp in SECONDS. 0 means unscheduled. A negative value is clamped to 0. It only matters for a post whose status is scheduled.
	ScheduleAt *int64 `json:"scheduleAt,omitempty"`
	// Status is the post's lifecycle state: draft, scheduled, published or failed. Omitted means draft. The transient publishing claim is never settable here — accepting it from a request would let a caller wedge or replay the guard that stops two publishers double-posting the same row.
	Status *string `json:"status,omitempty"`
}

// NewSocialPostBody instantiates a new SocialPostBody object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSocialPostBody() *SocialPostBody {
	this := SocialPostBody{}
	return &this
}

// NewSocialPostBodyWithDefaults instantiates a new SocialPostBody object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSocialPostBodyWithDefaults() *SocialPostBody {
	this := SocialPostBody{}
	return &this
}

// GetChannel returns the Channel field value if set, zero value otherwise.
func (o *SocialPostBody) GetChannel() string {
	if o == nil || IsNil(o.Channel) {
		var ret string
		return ret
	}
	return *o.Channel
}

// GetChannelOk returns a tuple with the Channel field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SocialPostBody) GetChannelOk() (*string, bool) {
	if o == nil || IsNil(o.Channel) {
		return nil, false
	}
	return o.Channel, true
}

// HasChannel returns a boolean if a field has been set.
func (o *SocialPostBody) HasChannel() bool {
	if o != nil && !IsNil(o.Channel) {
		return true
	}

	return false
}

// SetChannel gets a reference to the given string and assigns it to the Channel field.
func (o *SocialPostBody) SetChannel(v string) {
	o.Channel = &v
}

// GetContent returns the Content field value if set, zero value otherwise.
func (o *SocialPostBody) GetContent() string {
	if o == nil || IsNil(o.Content) {
		var ret string
		return ret
	}
	return *o.Content
}

// GetContentOk returns a tuple with the Content field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SocialPostBody) GetContentOk() (*string, bool) {
	if o == nil || IsNil(o.Content) {
		return nil, false
	}
	return o.Content, true
}

// HasContent returns a boolean if a field has been set.
func (o *SocialPostBody) HasContent() bool {
	if o != nil && !IsNil(o.Content) {
		return true
	}

	return false
}

// SetContent gets a reference to the given string and assigns it to the Content field.
func (o *SocialPostBody) SetContent(v string) {
	o.Content = &v
}

// GetMedia returns the Media field value if set, zero value otherwise.
func (o *SocialPostBody) GetMedia() []string {
	if o == nil || IsNil(o.Media) {
		var ret []string
		return ret
	}
	return o.Media
}

// GetMediaOk returns a tuple with the Media field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SocialPostBody) GetMediaOk() ([]string, bool) {
	if o == nil || IsNil(o.Media) {
		return nil, false
	}
	return o.Media, true
}

// HasMedia returns a boolean if a field has been set.
func (o *SocialPostBody) HasMedia() bool {
	if o != nil && !IsNil(o.Media) {
		return true
	}

	return false
}

// SetMedia gets a reference to the given []string and assigns it to the Media field.
func (o *SocialPostBody) SetMedia(v []string) {
	o.Media = v
}

// GetScheduleAt returns the ScheduleAt field value if set, zero value otherwise.
func (o *SocialPostBody) GetScheduleAt() int64 {
	if o == nil || IsNil(o.ScheduleAt) {
		var ret int64
		return ret
	}
	return *o.ScheduleAt
}

// GetScheduleAtOk returns a tuple with the ScheduleAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SocialPostBody) GetScheduleAtOk() (*int64, bool) {
	if o == nil || IsNil(o.ScheduleAt) {
		return nil, false
	}
	return o.ScheduleAt, true
}

// HasScheduleAt returns a boolean if a field has been set.
func (o *SocialPostBody) HasScheduleAt() bool {
	if o != nil && !IsNil(o.ScheduleAt) {
		return true
	}

	return false
}

// SetScheduleAt gets a reference to the given int64 and assigns it to the ScheduleAt field.
func (o *SocialPostBody) SetScheduleAt(v int64) {
	o.ScheduleAt = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *SocialPostBody) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SocialPostBody) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *SocialPostBody) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *SocialPostBody) SetStatus(v string) {
	o.Status = &v
}

func (o SocialPostBody) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SocialPostBody) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Channel) {
		toSerialize["channel"] = o.Channel
	}
	if !IsNil(o.Content) {
		toSerialize["content"] = o.Content
	}
	if !IsNil(o.Media) {
		toSerialize["media"] = o.Media
	}
	if !IsNil(o.ScheduleAt) {
		toSerialize["scheduleAt"] = o.ScheduleAt
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return toSerialize, nil
}

type NullableSocialPostBody struct {
	value *SocialPostBody
	isSet bool
}

func (v NullableSocialPostBody) Get() *SocialPostBody {
	return v.value
}

func (v *NullableSocialPostBody) Set(val *SocialPostBody) {
	v.value = val
	v.isSet = true
}

func (v NullableSocialPostBody) IsSet() bool {
	return v.isSet
}

func (v *NullableSocialPostBody) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSocialPostBody(val *SocialPostBody) *NullableSocialPostBody {
	return &NullableSocialPostBody{value: val, isSet: true}
}

func (v NullableSocialPostBody) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSocialPostBody) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
