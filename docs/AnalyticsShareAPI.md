...
------------- | ------------- | -------------
[**AnalyticsGetSharedWebsite**](AnalyticsShareAPI.md#AnalyticsGetSharedWebsite) | **Get** /v1/analytics/share/{shareId} | Get a shared website by share ID (no auth required)
...
func main() {
	...
	resp, r, err := apiClient.AnalyticsShareAPI.AnalyticsGetSharedWebsite(context.Background(), shareId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AnalyticsShareAPI.AnalyticsGetSharedWebsite``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `AnalyticsGetSharedWebsite`: AnalyticsGetSharedWebsite200Response
	fmt.Fprintf(os.Stdout, "Response from `AnalyticsShareAPI.AnalyticsGetSharedWebsite`: %v\n", resp)
}
```
