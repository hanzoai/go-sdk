...
------------- | ------------- | -------------
[**GatewayCreateCompletion**](GatewayCompletionsAPI.md#GatewayCreateCompletion) | **Post** /v1/gateway/completions | Create completion
...
func main() {
	...
	resp, r, err := apiClient.GatewayCompletionsAPI.GatewayCreateCompletion(context.Background()).GatewayCreateCompletionRequest(gatewayCreateCompletionRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `GatewayCompletionsAPI.GatewayCreateCompletion``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GatewayCreateCompletion`: map[string]interface{}
	fmt.Fprintf(os.Stdout, "Response from `GatewayCompletionsAPI.GatewayCreateCompletion`: %v\n", resp)
}
```
