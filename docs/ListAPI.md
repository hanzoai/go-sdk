...
func main() {
	...
	resp, r, err := apiClient.ListAPI.KvListPop(context.Background(), key).Namespace(namespace).KvListPopRequest(kvListPopRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ListAPI.KvListPop``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `KvListPop`: KvListPop200Response
	fmt.Fprintf(os.Stdout, "Response from `ListAPI.KvListPop`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ListAPI.KvListPush(context.Background(), key).KvListPushRequest(kvListPushRequest).Namespace(namespace).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ListAPI.KvListPush``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `KvListPush`: KvListPush200Response
	fmt.Fprintf(os.Stdout, "Response from `ListAPI.KvListPush`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ListAPI.KvListRange(context.Background(), key).Start(start).Stop(stop).Namespace(namespace).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ListAPI.KvListRange``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `KvListRange`: KvListRange200Response
	fmt.Fprintf(os.Stdout, "Response from `ListAPI.KvListRange`: %v\n", resp)
}
```
