...
**Conversions** | Pointer to **int32** | Conversions is how many of those signups have actually produced positive commission for the caller. Also derived, from the accrual rows, so it is ≤ signups and lags a referral until the first sweep after it spends. | [optional] 
**CreatedAt** | Pointer to **int32** | CreatedAt is when the link was minted, Unix seconds UTC. | [optional] 
...
**Signups** | Pointer to **int32** | Signups is how many orgs were attributed with this code — DERIVED by counting attribution edges, never stored, so it cannot drift from the ledger. | [optional] 
