...
------------- | ------------- | -------------
[**ExecExecCode**](ExecExecAPI.md#ExecExecCode) | **Post** /v1/exec | Execute code in a sandboxed session
[**ExecExecProgrammatic**](ExecExecAPI.md#ExecExecProgrammatic) | **Post** /v1/exec/programmatic | Programmatic code execution (sibling of /v1/exec, same executor contract)
...
func main() {
	...
	resp, r, err := apiClient.ExecExecAPI.ExecExecCode(context.Background()).ExecExecRequest(execExecRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExecExecAPI.ExecExecCode``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ExecExecCode`: ExecExecResult
	fmt.Fprintf(os.Stdout, "Response from `ExecExecAPI.ExecExecCode`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ExecExecAPI.ExecExecProgrammatic(context.Background()).ExecExecRequest(execExecRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ExecExecAPI.ExecExecProgrammatic``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ExecExecProgrammatic`: ExecExecResult
	fmt.Fprintf(os.Stdout, "Response from `ExecExecAPI.ExecExecProgrammatic`: %v\n", resp)
}
```
