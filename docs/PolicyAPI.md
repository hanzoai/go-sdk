...
func main() {
	...
	resp, r, err := apiClient.PolicyAPI.PricingGetPricingPolicy(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `PolicyAPI.PricingGetPricingPolicy``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `PricingGetPricingPolicy`: PricingPricingPolicy
	fmt.Fprintf(os.Stdout, "Response from `PolicyAPI.PricingGetPricingPolicy`: %v\n", resp)
}
```
