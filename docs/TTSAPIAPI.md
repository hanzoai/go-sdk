...
func main() {
	...
	resp, r, err := apiClient.TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudio(context.Background()).Body(body).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudio``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CloudApiControllerGenerateTextToSpeechAudio`: []string
	fmt.Fprintf(os.Stdout, "Response from `TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudio`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudioStream(context.Background()).StoreId(storeId).MessageId(messageId).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudioStream``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CloudApiControllerGenerateTextToSpeechAudioStream`: map[string]interface{}
	fmt.Fprintf(os.Stdout, "Response from `TTSAPIAPI.CloudApiControllerGenerateTextToSpeechAudioStream`: %v\n", resp)
}
```
