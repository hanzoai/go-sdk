...
func main() {
	...
	resp, r, err := apiClient.SpendAPI.GatewayCalculateSpend(context.Background()).GatewayCalculateSpendRequest(gatewayCalculateSpendRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SpendAPI.GatewayCalculateSpend``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GatewayCalculateSpend`: GatewayCalculateSpend200Response
	fmt.Fprintf(os.Stdout, "Response from `SpendAPI.GatewayCalculateSpend`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.SpendAPI.GatewayGetSpendLogs(context.Background()).UserId(userId).TeamId(teamId).StartDate(startDate).EndDate(endDate).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SpendAPI.GatewayGetSpendLogs``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GatewayGetSpendLogs`: []GatewayGetSpendLogs200ResponseInner
	fmt.Fprintf(os.Stdout, "Response from `SpendAPI.GatewayGetSpendLogs`: %v\n", resp)
}
```
