...
**CreatedAt** | Pointer to **int64** | CreatedAt is when the post was created, as a unix timestamp in seconds. | [optional] 
...
**ScheduleAt** | Pointer to **int64** | ScheduleAt is when the post is due, as a unix timestamp in SECONDS. 0 means unscheduled. It is meaningful only while the status is scheduled — a scheduled post whose time has arrived is published by the scheduler. | [optional] 
...
**UpdatedAt** | Pointer to **int64** | UpdatedAt is when the post row last changed, as a unix timestamp in seconds. The listing is ordered by it, newest first. | [optional] 
