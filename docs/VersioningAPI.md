...
func main() {
	...
	resp, r, err := apiClient.VersioningAPI.S3GetBucketVersioning(context.Background(), bucket).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `VersioningAPI.S3GetBucketVersioning``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `S3GetBucketVersioning`: S3VersioningConfig
	fmt.Fprintf(os.Stdout, "Response from `VersioningAPI.S3GetBucketVersioning`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	r, err := apiClient.VersioningAPI.S3PutBucketVersioning(context.Background(), bucket).S3VersioningConfig(s3VersioningConfig).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `VersioningAPI.S3PutBucketVersioning``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
}
```
