...
------------- | ------------- | -------------
[**AutomationsMcp**](AutomationsMCPAPI.md#AutomationsMcp) | **Post** /v1/automations/mcp | JSON-RPC 2.0 tool surface over connector actions
...
func main() {
	...
	resp, r, err := apiClient.AutomationsMCPAPI.AutomationsMcp(context.Background()).AutomationsMcpRequest(automationsMcpRequest).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AutomationsMCPAPI.AutomationsMcp``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `AutomationsMcp`: AutomationsMcpResponse
	fmt.Fprintf(os.Stdout, "Response from `AutomationsMCPAPI.AutomationsMcp`: %v\n", resp)
}
```
