...
------------- | ------------- | -------------
[**AdminAdminListAudit**](AdminAuditAPI.md#AdminAdminListAudit) | **Get** /v1/admin/audit | Query the tamper-evident audit trail
[**AdminAdminVerifyAudit**](AdminAuditAPI.md#AdminAdminVerifyAudit) | **Get** /v1/admin/audit/verify | Verify audit-chain integrity
...
func main() {
	...
	resp, r, err := apiClient.AdminAuditAPI.AdminAdminListAudit(context.Background()).Org(org).Sub(sub).Action(action).Resource(resource).Result(result).Since(since).Until(until).PageSize(pageSize).P(p).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AdminAuditAPI.AdminAdminListAudit``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `AdminAdminListAudit`: AdminAdminListAudit200Response
	fmt.Fprintf(os.Stdout, "Response from `AdminAuditAPI.AdminAdminListAudit`: %v\n", resp)
}
```
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.AdminAuditAPI.AdminAdminVerifyAudit(context.Background()).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AdminAuditAPI.AdminAdminVerifyAudit``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `AdminAdminVerifyAudit`: AdminAdminVerifyAudit200Response
	fmt.Fprintf(os.Stdout, "Response from `AdminAuditAPI.AdminAdminVerifyAudit`: %v\n", resp)
}
```
