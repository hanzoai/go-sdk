...
------------- | ------------- | -------------
[**DnsGetZoneAnalytics**](DnsAnalyticsAPI.md#DnsGetZoneAnalytics) | **Get** /v1/dns/zones/{zone}/analytics | Get query analytics
...
func main() {
	...
	resp, r, err := apiClient.DnsAnalyticsAPI.DnsGetZoneAnalytics(context.Background(), zone).From(from).To(to).Granularity(granularity).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DnsAnalyticsAPI.DnsGetZoneAnalytics``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `DnsGetZoneAnalytics`: DnsQueryAnalytics
	fmt.Fprintf(os.Stdout, "Response from `DnsAnalyticsAPI.DnsGetZoneAnalytics`: %v\n", resp)
}
```
