...
type DestinationAPIDeleteDestinationByPlatformRequest struct {
	ctx        context.Context
...
/*
DeleteDestinationByPlatform Forgets a destination for the caller's org: every credential held in KMS, then the stored config.
...
func (a *DestinationAPIService) DeleteDestinationByPlatformExecute(r DestinationAPIDeleteDestinationByPlatformRequest) (*DestinationDisconnected, *http.Response, error) {
	...
	localVarPath := localBasePath + "/v1/destination/{platform}"
...
type DestinationAPIGetDestinationRequest struct {
	ctx        context.Context
	ApiService *DestinationAPIService
}
...
/*
GetDestination Reports every destination this deployment can forward to, each with the caller org's connection state: whether it is connected, whether it is enabled, whether a credential resolves right now, and the config fields the console renders for it.
...
renders for it.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return DestinationAPIGetDestinationRequest
*/
func (a *DestinationAPIService) GetDestination(ctx context.Context) DestinationAPIGetDestinationRequest {
	return DestinationAPIGetDestinationRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
func (a *DestinationAPIService) GetDestinationExecute(r DestinationAPIGetDestinationRequest) (*DestinationList, *http.Response, error) {
	...
	localVarPath := localBasePath + "/v1/destination"
...
type DestinationAPIGetDestinationByPlatformRequest struct {
	ctx        context.Context
	ApiService *DestinationAPIService
	platform   string
}
...
/*
GetDestinationByPlatform Reports one destination's card for the caller's org — its config fields, its connection state, and whether a credential resolves right now.
...
type DestinationAPIPostDestinationByPlatformRequest struct {
	ctx         context.Context
	ApiService  *DestinationAPIService
	platform    string
...
func (r DestinationAPIPostDestinationByPlatformRequest) RequestBody(requestBody map[string]interface{}) DestinationAPIPostDestinationByPlatformRequest {
	r.requestBody = &requestBody
	return r
}

func (r DestinationAPIPostDestinationByPlatformRequest) Execute() (*DestinationStatus, *http.Response, error) {
	return r.ApiService.PostDestinationByPlatformExecute(r)
}

/*
PostDestinationByPlatform Connect one conversion destination for your org, or update the one you have
...
Stores the addressed platform's non-secret ids (its measurement, pixel or dataset ids) and seals its API credential into KMS under a path scoped to the caller's own org, then answers the same status card the read routes do — with live telling you whether the credential actually resolves right now. The body's property NAMES are the platform's own: each field the platform declares, plus each secret under its camelCase name, so the accepted keys differ per platform and a missing REQUIRED field is refused. Connecting is an ORG ADMIN action — a validated member without the admin bit gets 403 — and it fails closed with 503 when the KMS master key is unavailable rather than persisting a destination whose secret was never sealed. The secret itself never appears in the response, in the store, or in a log line; only its NAME is ever published. Set enabled to false to keep the connection but stop the analytics fan-out to it.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param platform
	@return DestinationAPIPostDestinationByPlatformRequest
*/
func (a *DestinationAPIService) PostDestinationByPlatform(ctx context.Context, platform string) DestinationAPIPostDestinationByPlatformRequest {
	return DestinationAPIPostDestinationByPlatformRequest{
		ApiService: a,
		ctx:        ctx,
		platform:   platform,
...
//
//	@return DestinationStatus
func (a *DestinationAPIService) PostDestinationByPlatformExecute(r DestinationAPIPostDestinationByPlatformRequest) (*DestinationStatus, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "DestinationAPIService.PostDestinationByPlatform")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}
...
type DestinationAPIPostDestinationByPlatformTestRequest struct {
	ctx        context.Context
	ApiService *DestinationAPIService
	platform   string
}
...
func (a *DestinationAPIService) PostDestinationByPlatformTestExecute(r DestinationAPIPostDestinationByPlatformTestRequest) (*DestinationTest, *http.Response, error) {
	...
	localVarPath := localBasePath + "/v1/destination/{platform}/test"
	localVarPath = strings.Replace(localVarPath, "{"+"platform"+"}", url.PathEscape(parameterValueToString(r.platform, "platform")), -1)
