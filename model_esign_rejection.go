/*
Hanzo Cloud API

The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"encoding/json"
)

// checks if the EsignRejection type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &EsignRejection{}

// EsignRejection struct for EsignRejection
type EsignRejection struct {
	// RecipientID is the recipient who declined.
	RecipientId *string `json:"recipientId,omitempty"`
	// Status is REJECTED — one declining signer ends the document for everyone, and there is no route back.
	Status *string `json:"status,omitempty"`
}

// NewEsignRejection instantiates a new EsignRejection object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewEsignRejection() *EsignRejection {
	this := EsignRejection{}
	return &this
}

// NewEsignRejectionWithDefaults instantiates a new EsignRejection object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewEsignRejectionWithDefaults() *EsignRejection {
	this := EsignRejection{}
	return &this
}

// GetRecipientId returns the RecipientId field value if set, zero value otherwise.
func (o *EsignRejection) GetRecipientId() string {
	if o == nil || IsNil(o.RecipientId) {
		var ret string
		return ret
	}
	return *o.RecipientId
}

// GetRecipientIdOk returns a tuple with the RecipientId field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EsignRejection) GetRecipientIdOk() (*string, bool) {
	if o == nil || IsNil(o.RecipientId) {
		return nil, false
	}
	return o.RecipientId, true
}

// HasRecipientId returns a boolean if a field has been set.
func (o *EsignRejection) HasRecipientId() bool {
	if o != nil && !IsNil(o.RecipientId) {
		return true
	}

	return false
}

// SetRecipientId gets a reference to the given string and assigns it to the RecipientId field.
func (o *EsignRejection) SetRecipientId(v string) {
	o.RecipientId = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *EsignRejection) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *EsignRejection) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *EsignRejection) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *EsignRejection) SetStatus(v string) {
	o.Status = &v
}

func (o EsignRejection) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o EsignRejection) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.RecipientId) {
		toSerialize["recipientId"] = o.RecipientId
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	return toSerialize, nil
}

type NullableEsignRejection struct {
	value *EsignRejection
	isSet bool
}

func (v NullableEsignRejection) Get() *EsignRejection {
	return v.value
}

func (v *NullableEsignRejection) Set(val *EsignRejection) {
	v.value = val
	v.isSet = true
}

func (v NullableEsignRejection) IsSet() bool {
	return v.isSet
}

func (v *NullableEsignRejection) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableEsignRejection(val *EsignRejection) *NullableEsignRejection {
	return &NullableEsignRejection{value: val, isSet: true}
}

func (v NullableEsignRejection) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableEsignRejection) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
