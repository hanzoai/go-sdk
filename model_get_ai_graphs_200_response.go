/*
Hanzo Cloud API

The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator's admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.

API version: v1
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package hanzoai

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the GetAiGraphs200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetAiGraphs200Response{}

// GetAiGraphs200Response struct for GetAiGraphs200Response
type GetAiGraphs200Response struct {
	Data  []Graph     `json:"data,omitempty"`
	Data2 interface{} `json:"data2,omitempty"`
	// Empty on success, the reason on failure.
	Msg    string `json:"msg"`
	Status string `json:"status"`
}

type _GetAiGraphs200Response GetAiGraphs200Response

// NewGetAiGraphs200Response instantiates a new GetAiGraphs200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetAiGraphs200Response(msg string, status string) *GetAiGraphs200Response {
	this := GetAiGraphs200Response{}
	this.Msg = msg
	this.Status = status
	return &this
}

// NewGetAiGraphs200ResponseWithDefaults instantiates a new GetAiGraphs200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetAiGraphs200ResponseWithDefaults() *GetAiGraphs200Response {
	this := GetAiGraphs200Response{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *GetAiGraphs200Response) GetData() []Graph {
	if o == nil || IsNil(o.Data) {
		var ret []Graph
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetAiGraphs200Response) GetDataOk() ([]Graph, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *GetAiGraphs200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []Graph and assigns it to the Data field.
func (o *GetAiGraphs200Response) SetData(v []Graph) {
	o.Data = v
}

// GetData2 returns the Data2 field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *GetAiGraphs200Response) GetData2() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.Data2
}

// GetData2Ok returns a tuple with the Data2 field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *GetAiGraphs200Response) GetData2Ok() (*interface{}, bool) {
	if o == nil || IsNil(o.Data2) {
		return nil, false
	}
	return &o.Data2, true
}

// HasData2 returns a boolean if a field has been set.
func (o *GetAiGraphs200Response) HasData2() bool {
	if o != nil && !IsNil(o.Data2) {
		return true
	}

	return false
}

// SetData2 gets a reference to the given interface{} and assigns it to the Data2 field.
func (o *GetAiGraphs200Response) SetData2(v interface{}) {
	o.Data2 = v
}

// GetMsg returns the Msg field value
func (o *GetAiGraphs200Response) GetMsg() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Msg
}

// GetMsgOk returns a tuple with the Msg field value
// and a boolean to check if the value has been set.
func (o *GetAiGraphs200Response) GetMsgOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Msg, true
}

// SetMsg sets field value
func (o *GetAiGraphs200Response) SetMsg(v string) {
	o.Msg = v
}

// GetStatus returns the Status field value
func (o *GetAiGraphs200Response) GetStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *GetAiGraphs200Response) GetStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *GetAiGraphs200Response) SetStatus(v string) {
	o.Status = v
}

func (o GetAiGraphs200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetAiGraphs200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if o.Data2 != nil {
		toSerialize["data2"] = o.Data2
	}
	toSerialize["msg"] = o.Msg
	toSerialize["status"] = o.Status
	return toSerialize, nil
}

func (o *GetAiGraphs200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"msg",
		"status",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varGetAiGraphs200Response := _GetAiGraphs200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varGetAiGraphs200Response)

	if err != nil {
		return err
	}

	*o = GetAiGraphs200Response(varGetAiGraphs200Response)

	return err
}

type NullableGetAiGraphs200Response struct {
	value *GetAiGraphs200Response
	isSet bool
}

func (v NullableGetAiGraphs200Response) Get() *GetAiGraphs200Response {
	return v.value
}

func (v *NullableGetAiGraphs200Response) Set(val *GetAiGraphs200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetAiGraphs200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetAiGraphs200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetAiGraphs200Response(val *GetAiGraphs200Response) *NullableGetAiGraphs200Response {
	return &NullableGetAiGraphs200Response{value: val, isSet: true}
}

func (v NullableGetAiGraphs200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetAiGraphs200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
